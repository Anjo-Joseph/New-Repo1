    ...
    function (search, runtime, format, formati, file, record) {
        ...
        function onRequestFxn(context) {
            try {
                ...
                var currentuser = context.request.parameters.custInternalId?context.request.parameters.custInternalId:runtime.getCurrentUser().id;
                ...
                if (context.request.method === 'POST') {
                    ...
                    log.error('post startDate', startdate_post);
                    var enddate_post = checkForParameter(context.request.parameters.enddate, "");
                    log.error('post enddate_post', enddate_post);
                    ...
                    var content =
                        ...
                        'if(startDate=="" && endDate==""){' +
                        'window.location.href=("/app/site/hosting/scriptlet.nl?script=1858&deploy=1&custInternalId="+currentuser+"&startdate="+startDate+"&enddate="+endDate);' +
                        '}' +
